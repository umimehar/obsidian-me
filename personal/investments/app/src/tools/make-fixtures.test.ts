...
function config(overrides: Partial<Config> = {}): Config {
  return {
    redactions: ["Jane Doe"],
    addressWords: ["1200-300", "Springfield"],
...
describe("phraseTokens", () => {
  test("keeps a hyphenated address token whole, and also splits it into halves", () => {
    ...
    const tokens = phraseTokens("1200-300");
    expect(tokens).toContain("1200-300");
    expect(tokens).toContain("1200");
    expect(tokens).toContain("300");
  });
...
describe("scrub", () => {
  ...
  test("does not redact an unrelated number that happens to contain a civic/unit digit sequence", () => {
    // "300" is a token from the "1200-300" address, but a dollar figure like
    // "$3,300.00" is a different, whole bbox word -- scrub compares the
    // punctuation-stripped WHOLE word, not a substring.
    const xml = xmlWord("$3,300.00");
    const out = scrub(xml, "ACCT0001CAD", "ACCT9999CAD", config());
    expect(out).toContain("$3,300.00");
...
describe("assertClean", () => {
  ...
  test("does not throw on an unrelated word that merely contains the token as a substring", () => {
    ...
    // address token like "300" false-positived against any unrelated number
    // containing "300" anywhere on the page.
    const xml = xmlWord("$3,300.00");
