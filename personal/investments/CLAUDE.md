...
- Room / contribution figures in the Contributions and Room section use `contrib` (the CONTRIB-tagged contributions), summed for the scoped accounts of each registered group in the tax year (`taxSummary` in `src/client/series.ts`). Reference figure: RRSP `contrib` for 2026 is $25,000, for 2025 $3,000 ($28,000 total). This deliberately excludes routing-hub deposits, so it stays under the annual limit and matches what actually landed as contributions.
...
- Room bars work the same way: `used` is `contrib` summed for the selected accounts of each registered group (TFSA, FHSA, RRSP, RESP; ManagedTFSA shares the TFSA group) within the scope's tax year, against that year's CRA limit. `contrib` (not gross `external_in`) is used because a routing hub account inflates `external_in` with pass-through deposits. There is no OVER flag — unused room carries forward from prior years, so a full or over-full bar is not necessarily an over-contribution.
