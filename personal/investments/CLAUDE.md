...
- `series[].external_in` and `series[].external_out` are the classified external money movements, computed by `externalFlow` in `src/analytics.ts` (see "External versus internal transfers" below). `external_in` is the contribution basis, the money added from outside, including CONTRIB plus every external deposit that Wealthsimple coded as TRANSFER_IN. It is GROSS per account and double counts at the portfolio aggregate when money is routed through a hub account, so it is used only for per-account contributions and room, never for total money in.
